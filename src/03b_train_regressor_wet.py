#!/usr/bin/env python3
"""Train an XGBoost regressor on wet days only (PRATE > 1e-5)."""
...
def load_h5_sample_wet_only(
    path: Path, max_rows: int | None, seed: int
) -> tuple[np.ndarray, np.ndarray, list[str]]:
    """Load HDF5 data, filter to wet days only (y_raw > 1e-5), then subsample if needed."""
    with h5py.File(path, "r") as h5:
        feature_names = json.loads(h5.attrs["feature_names_json"])

        X_all = h5["X"][:].astype(np.float32)
        y_raw_all = h5["y_raw"][:].astype(np.float32)
        y_log_all = h5["y_log"][:].astype(np.float32)

        # Filter to wet days only
        wet_mask = y_raw_all > 1e-6
        X_wet = X_all[wet_mask]
        y_wet = y_log_all[wet_mask]

        n_wet = X_wet.shape[0]

        # Subsample if needed
        if max_rows is None or max_rows >= n_wet:
            idx = np.arange(n_wet)
        else:
            rng = np.random.default_rng(seed)
            idx = np.sort(rng.choice(n_wet, size=max_rows, replace=False))

        X = X_wet[idx].astype(np.float32)
        y = y_wet[idx].astype(np.float32)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Train XGBoost regressor on wet days only"
    )
    parser.add_argument("--train-file", default="data/train.h5")
    parser.add_argument("--val-file", default="data/val.h5")
    parser.add_argument("--model-out", default="models/xgb_regressor_wet.json")
    parser.add_argument(
        "--importance-out", default="models/regressor_wet_importance.csv"
    )
    ...
    print(f"Loading training data (wet days only) from {train_file}")
    X_train, y_train, feature_names = load_h5_sample_wet_only(
        train_file,
        max_rows=args.max_train_rows,
        seed=args.seed,
    )
    print(
        f"Loaded train (wet only): {X_train.shape[0]:,} rows, {X_train.shape[1]} features"
    )

    print(f"Loading validation data (wet days only) from {val_file}")
    X_val, y_val, _ = load_h5_sample_wet_only(
        val_file,
        max_rows=args.max_val_rows,
        seed=args.seed + 1,
    )
    print(f"Loaded val (wet only): {X_val.shape[0]:,} rows")
    ...
    print("Validation metrics (wet days only):")
